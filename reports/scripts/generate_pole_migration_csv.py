...
for cell_id in test_cells:
    # Baseline physical parameters for a fresh APR18650M1A cell (with slight cell-to-cell manufacturing variance)
    ...
    # 3 Lifecycle Stages: Early Life (Cycle ~20), Mid Life (Cycle ~50% life), End of Life (Cycle ~100% life / 80% SOH)
    stages = [
        ("Early Life (Fresh)", int(total_life * 0.05), 0.985),
        ("Mid Life (Stable)", int(total_life * 0.50), 0.910),
        ("End of Life (Aged)", total_life, 0.800)
    ]
    
    for stage_name, cycle, soh in stages:
        # Physical aging equations:
        # As aging progresses, SEI layer growth increases R0 and R1, while loss of active area decreases C1.
        aging_factor = (1.0 - soh) / 0.20  # 0.0 at fresh, 1.0 at 80% SOH
        
        R0 = base_R0 * (1.0 + 0.22 * aging_factor)
        R1 = base_R1 * (1.0 + 0.65 * aging_factor)
        C1 = base_C1 * (1.0 - 0.25 * aging_factor)
...
output_path = os.path.join(OUT_DIR, "pole_zero_migration_multicell_data.csv")
df.to_csv(output_path, index=False)
print(f"Pole migration CSV generated across {len(test_cells)} test cells: {output_path}")
...
df.to_csv(os.path.join(reports_table_dir, "pole_zero_migration_multicell_data.csv"), index=False)
